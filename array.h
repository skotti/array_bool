...
public:
	...
	/*std::ptrdiff_t operator-(Array<bool>::iterator it1, Array<bool>::iterator it2) {
	 return (it2.m_offset - it1.m_offset);
	}
	
	std::ptrdiff_t operator-(Array<bool>::const_iterator it1, Array<bool>::const_iterator it2) const{
	 return (it2.m_offset - it1.m_offset);
	}*/
	...
	Array(const bool* data, int size) {
		...
		m_data = static_cast<char*>(operator new[] (cur_size));
	...
	void reserve(int size) {
		...
		if (size > m_capacity) {
			...
			m_data = static_cast<char*>(operator new[] (cur_size/8));
